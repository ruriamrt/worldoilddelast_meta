...
Chunk 11: g8.Metareg  (metaregression analysis: WLS and LME specifications; diagnostic tests)
Chunk 12: outFunnelPlots
Chunk 13: out.Tables_LRT
Chunk 14: out.Tables_PSRTransp
Chunk 15: out.Tables_PSROther
Chunk 16: out.Tables_PLRTransp
Chunk 17: out.Tables_PLROther
Chunk 18: out.Tables_YSRTransp
Chunk 19: out.Tables_YSROther
Chunk 20: out.Tables_YLRTransp
Chunk 21: out.Tables_YLROther
Chunk 22: out.tbl.baselines
Chunk 23: g8.world_elast (constructs oil demand world price-elasticity as weighted average of the various sectors)
Chunk 24: g8.IncomeEffects (incorporates income effects to the oil demand price-elasticity)
Chunk 25: outworldelastplot
Chunk 26: WorldElast.compare (comparison plot between short-run world oil demand price elasticity from metaregressions and other published estimates)
Chunk 27: out_elastcompare
-->
...
allelast <- allelast %>%
  mutate(RoadTransp = ifelse(PROD_SECT %in% c("Gasoline", "Diesel", "Transportation"), 1, 0),
...
allelast <- allelast %>%
  mutate(g8.weights.RE = 1/(ST_ERROR**2 + g8.tau),
         g8.tau.frac = g8.tau/g8.weights.RE)
...
g8.avg.summary <- g8.avg.summary %>%
  left_join(g8.ElastIDmatch, by = c("Type_Sector")) %>%
  left_join(g8.avg_ALL[,c("Type_Sector","unweighted_mean")], by=c("Type_Sector")) %>%
  mutate(g8.ElastID = as.factor(g8.ElastID),
...
for (i in 1:nrow(g8.selectorSets)) {
  ...
  g8.curr_varList = ifelse(g8.selectorSets[i,1] == "P_SR" & g8.selectorSets[i,2] == "1" & g8.selectorSets[i,3] == "0", list(all_varList_P_SR.Transp),
                     ifelse(g8.selectorSets[i,1] =="P_SR" & g8.selectorSets[i,2] == "0" & g8.selectorSets[i,3] == "1", list(all_varList_P_SR.nonTransp),
                      ifelse(g8.selectorSets[i,1] =="P_LR" & g8.selectorSets[i,2] == "1" & g8.selectorSets[i,3] == "0", list(all_varList_P_LR.Transp),
                       ifelse(g8.selectorSets[i,1] =="P_LR" & g8.selectorSets[i,2] == "0" & g8.selectorSets[i,3] == "1", list(all_varList_P_LR.nonTransp),
                        ifelse(g8.selectorSets[i,1] =="Y_SR" & g8.selectorSets[i,2] == "1" & g8.selectorSets[i,3] == "0", list(all_varList_Y_SR.Transp),
                         ifelse(g8.selectorSets[i,1] =="Y_SR" & g8.selectorSets[i,2] == "0" & g8.selectorSets[i,3] == "1", list(all_varList_Y_SR.nonTransp),
                          ifelse(g8.selectorSets[i,1] =="Y_LR" & g8.selectorSets[i,2] == "1" & g8.selectorSets[i,3] == "0", list(all_varList_Y_LR.Transp),
                           ifelse(g8.selectorSets[i,1] =="Y_LR" & g8.selectorSets[i,2] == "0" & g8.selectorSets[i,3] == "1", list(all_varList_Y_LR.nonTransp), NA
                     ))))))))
...
g8.all_baselinesEstimated <- do.call(rbind,g8.baselinesEstimated) %>%
  as.data.frame %>%
...
g8.all_baselinesEstimated <- g8.all_baselinesEstimated %>%
  mutate(raw.coeffvar = raw.mean/raw.stdev,
...
g8.all_baselinesEstimated <- g8.all_baselinesEstimated %>%
  mutate(fittedmean = ifelse(LRT_pvalue < 0.05, fittedmean.LME, fittedmean.WLS),
...
g8.obsSel2.df <- do.call("rbind", g8.obsSel2) %>%
  ...
  left_join(g8.ElastIDmatch2, by="Type_Sector")
...
pub.bias.test <- do.call("rbind", g8.pubbias.test) %>%
  rename(Type_Sector = `rep(g8.subsetString2, 2)`) %>%
  left_join(g8.ElastIDmatch, by = "Type_Sector") %>%
  mutate(Type = as.factor(ifelse(substr(Type_Sector, 1, 1) == "P", "Price", "Income")),
         Length.of.Run = as.factor(ifelse(grepl("Long", g8.ElastID) == T, "Long Run", "Short Run")),
         Sector = as.factor(ifelse(grepl("non", g8.ElastID), "nonTransportation", "Transportation"))) %>%
  mutate_if(is.numeric, round, 3) %>%
  select(Type, Length.of.Run, Sector, estimate, std.error, statistic, g8.ElastID)
...
LRT.table[i,2] <- g8.LRT[[i]][1, "LogLik"] #log likelihood of the full model 
LRT.table[i,3] <- g8.LRT[[i]][2, "LogLik"] #log likelihood of the restricted model
LRT.table[i,4] <- g8.LRT[[i]][2, "Chisq"] #value of the statistic
LRT.table[i,5] <- g8.LRT[[i]][2, "Df"] #difference in degrees of freedom
LRT.table[i,6] <- g8.LRT[[i]][2, "Pr(>Chisq)"] #p-value of the statistic
LRT.table[i,7] <- g8.all_baselinesEstimated$n.papers[[i]] #number of clusters
}
...
LRT.table <- LRT.table %>%
  ...
  mutate_at((c(2:6)), round, 3) %>%
  mutate_at((c(5)), abs)
  
colnames(LRT.table) <- c("Grouping","Log.Lik.MLE", "Log.Lik.WLS", "Statistic", "Degrees.of.Freedom", "p.value", "number.clusters")

kable(LRT.table, digits = 3, colnames = c("Log Likelihood LME", "Log Likelihood WLS", "Statistic", "Degrees of Freedom", "p value", "Number of Clusters"))
```
...
# g8.worldelastw <- read_excel("../Data/auxiliary_inputs_20170829.xlsx", sheet = "cons.weights_IEO2016", col_names = TRUE, skip=4) %>%
...
#  
#  g8.worldelastw2 <- g8.worldelastw %>%
...
g8.out_summary <- g8.all_baselinesEstimated %>%
  select(subsetString, baseline, sterror.baseline, mean.SPEEDYRS) %>%
  mutate(Region = "1_1_",
         BaselineType = "crudep")

g8.out_summary.OECD <- g8.all_baselinesEstimated %>%
...
g8.out_summary.nonOECD <- g8.all_baselinesEstimated %>%
...
g8.out_summary.max <- g8.all_baselinesEstimated %>%
...
g8.out_summary.prodpmax <- g8.all_baselinesEstimated %>%
...
g8.out_summary1 <- rbind(g8.out_summary, g8.out_summary.OECD, g8.out_summary.nonOECD, g8.out_summary.max, g8.out_summary.prodpmax) %>%
  mutate(run = as.factor(substr(subsetString, 3, 4)),
...
g8.out_summary2 <-  g8.out_summary1 %>%
...
g8.out_summary3 <- g8.out_summary2 %>%
  mutate(slope = (LR_baseline - SR_baseline)/(LR_mean.SPEEDYRS - SR_mean.SPEEDYRS),
...
g8.out_summary4 <- g8.out_summary3 %>%
  ...
  select(c(22,21,3:17)) %>%
  rename(LR_est.baseline = LR_baseline, 
...
write.csv(g8.out_summary4, "../Output/g8.elast_line_coeffs.csv")
...
g8.elast.sim <- matrix(c(0), ndraws, nrow(g8.out_summary3))
colnames(g8.elast.sim) = paste(g8.out_summary3$case2, g8.out_summary3$BaselineType, sep = "_")

for(i in 1:length(g8.intercept)){
#truncated normal distribution
  g8.elast.sim[,i] <- rtruncnorm(ndraws, 
                                 a = as.numeric(g8.intercept[i]) - 1 * as.numeric(g8.st_error[i]), 
                                 b = as.numeric(g8.intercept[i]) + 1 * as.numeric(g8.st_error[i]),  
                                 mean = as.numeric(g8.intercept[i]), 
                                 sd = as.numeric(g8.st_error[i]))
}
...
g8.world_P_SR <- matrix(c(0),ndraws,nyears) 
for(i in 1:nyears){
               g8.world_P_SR[,i] = 
...
g8.world_P_SR_crudep <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
g8.world_P_SR_crudepmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
g8.world_P_SR_prodpmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on regional-sectoral baselines
g8.world_P_LR <- matrix(c(0),ndraws,nyears) 
for(i in 1:nyears){
                g8.world_P_LR[,i] = 
               g8.worldelastw[g8.worldelastw$RegSect =="non_OECD_Non-transportation",c(i+2)]*
                 (g8.elast.sim[, "P_0_1_0_1_crudep"] + 
...
##based on sectoral baselines
g8.world_P_LR_crudep <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on sectoral baselines and for crude price maximum
g8.world_P_LR_crudepmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on sectoral baselines and for product price maximum
g8.world_P_LR_prodpmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on regional-sectoral baselines
g8.world_Y_SR <- matrix(c(0),ndraws,nyears) 
for(i in 1:nyears){
               g8.world_Y_SR[,i] = 
               g8.worldelastw[g8.worldelastw$RegSect =="non_OECD_Non-transportation",c(i+2)]*
...
##based on sectoral baselines
g8.world_Y_SR_crudep <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on sectoral baselines and for crude price maximum
g8.world_Y_SR_crudepmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on sectoral baselines and for product price maximum
g8.world_Y_SR_prodpmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on regional-sectoral baselines
g8.world_Y_LR <- matrix(c(0),ndraws,nyears) 
for(i in 1:nyears){
                g8.world_Y_LR[,i] = 
               g8.worldelastw[g8.worldelastw$RegSect =="non_OECD_Non-transportation",c(i+2)]*
                 (g8.elast.sim[, "Y_0_1_0_1_crudep"] + 
...
##based on sectoral baselines
g8.world_Y_LR_crudep <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on sectoral baselines and for crude price maximum
g8.world_Y_LR_crudepmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on sectoral baselines and for product price maximum
g8.world_Y_LR_prodpmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##collect the results in a data frame for plotting in ggplot2
 g8.P_SR_dat <- data.frame(Year = seq(2010,2040,1), lower = g8.est.world_P_SR[1,], middle = g8.est.world_P_SR[2,], upper = g8.est.world_P_SR[3,]) %>%
   mutate(TypeLength = "Short-Run Price",
 ...
 g8.P_LR_dat <- data.frame(Year = seq(2010,2040,1), lower = g8.est.world_P_LR[1,], middle = g8.est.world_P_LR[2,], upper = g8.est.world_P_LR[3,]) %>%
   mutate(TypeLength = "Long-Run Price",
          BaselineType = "crudep_regions")
 ...
 g8.Y_SR_dat <- data.frame(Year = seq(2010,2040,1), lower = g8.est.world_Y_SR[1,], middle = g8.est.world_Y_SR[2,], upper = g8.est.world_Y_SR[3,]) %>%
   mutate(TypeLength = "Short-Run Income",
          BaselineType = "crudep_regions")
 ...
 g8.Y_LR_dat <- data.frame(Year = seq(2010,2040,1), lower = g8.est.world_Y_LR[1,], middle = g8.est.world_Y_LR[2,], upper = g8.est.world_Y_LR[3,]) %>%
   mutate(TypeLength = "Long-Run Income",
          BaselineType = "crudep_regions")
...
##SHORT_RUN
g8.world_P_SR_withY <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){g8.world_P_SR_withY[,i] = g8.world_P_SR[,i] + GDPoilelast * g8.world_Y_SR[,i]}

g8.world_P_SR_withY_crudep <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){g8.world_P_SR_withY_crudep[,i] = g8.world_P_SR_crudep[,i] + GDPoilelast * g8.world_Y_SR_crudep[,i]}

g8.world_P_SR_withY_crudepmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){g8.world_P_SR_withY_crudepmax[,i] = g8.world_P_SR_crudepmax[,i] + GDPoilelast * g8.world_Y_SR_crudepmax[,i]}

g8.world_P_SR_withY_prodpmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){g8.world_P_SR_withY_prodpmax[,i] = g8.world_P_SR_prodpmax[,i] + GDPoilelast * g8.world_Y_SR_prodpmax[,i]}

##LONG_RUN
g8.world_P_LR_withY <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){g8.world_P_LR_withY[,i] = g8.world_P_LR[,i] + GDPoilelast * g8.world_Y_LR[,i]}

g8.world_P_LR_withY_crudep <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){g8.world_P_LR_withY_crudep[,i] = g8.world_P_LR_crudep[,i] + GDPoilelast * g8.world_Y_LR_crudep[,i]}

g8.world_P_LR_withY_crudepmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){g8.world_P_LR_withY_crudepmax[,i] = g8.world_P_LR_crudepmax[,i] + GDPoilelast * g8.world_Y_LR_crudepmax[,i]}

g8.world_P_LR_withY_prodpmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){g8.world_P_LR_withY_prodpmax[,i] = g8.world_P_LR_prodpmax[,i] + GDPoilelast * g8.world_Y_LR_prodpmax[,i]}
...
g8.world.elast.p.plot.withY <- ggplot(g8.all.worldelast.p.withY, aes(x=BaselineType, y=middle)) + 
                  facet_wrap(~TypeLength) +
                  geom_point(data=g8.all.worldelast.p.withY, aes(x=BaselineType, y=middle), color = "black") +
...
g8.out_summaryb <- g8.out_summary1 %>%
...
g8.elast.simb <- matrix(c(0), ndraws, nrow(g8.out_summaryb))
colnames(g8.elast.simb) = g8.out_summaryb$caserun

for(i in 1:length(g8.baseline)){
#truncated normal distribution
  g8.elast.simb[,i] <- rtruncnorm(ndraws, a = as.numeric(g8.baseline[i])- 1 * as.numeric(g8.sterror[i]), b = as.numeric(g8.baseline[i])+ 1* as.numeric(g8.sterror[i]), mean = as.numeric(g8.baseline[i]), sd = as.numeric(g8.sterror[i]))
}
...
##based on sectoral baselines
g8.world_PSR_crudep <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on sectoral baselines and for crude price maximum
g8.world_PSR_crudepmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on sectoral baselines and for product price maximum
g8.world_PSR_prodpmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on sectoral baselines
g8.world_PLR_crudep <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on sectoral baselines and for crude price maximum
g8.world_PLR_crudepmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on sectoral baselines and for product price maximum
g8.world_PLR_prodpmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on sectoral baselines
g8.world_YSR_crudep <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on sectoral baselines and for crude price maximum
g8.world_YSR_crudepmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on sectoral baselines and for product price maximum
g8.world_YSR_prodpmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on sectoral baselines
g8.world_YLR_crudep <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on sectoral baselines and for crude price maximum
g8.world_YLR_crudepmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##based on sectoral baselines and for product price maximum
g8.world_YLR_prodpmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){
...
##SHORT_RUN
g8.world_PSR_withY_crudep <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){g8.world_PSR_withY_crudep[,i] = g8.world_PSR_crudep[,i] + GDPoilelast * g8.world_YSR_crudep[,i]}

g8.world_PSR_withY_crudepmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){g8.world_PSR_withY_crudepmax[,i] = g8.world_PSR_crudepmax[,i] + GDPoilelast * g8.world_YSR_crudepmax[,i]}

g8.world_PSR_withY_prodpmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){g8.world_PSR_withY_prodpmax[,i] = g8.world_PSR_prodpmax[,i] + GDPoilelast * g8.world_YSR_prodpmax[,i]}

##LONG_RUN
g8.world_PLR_withY_crudep <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){g8.world_PLR_withY_crudep[,i] = g8.world_PLR_crudep[,i] + GDPoilelast * g8.world_YLR_crudep[,i]}

g8.world_PLR_withY_crudepmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){g8.world_PLR_withY_crudepmax[,i] = g8.world_PLR_crudepmax[,i] + GDPoilelast * g8.world_YLR_crudepmax[,i]}

g8.world_PLR_withY_prodpmax <- matrix(c(0),ndraws,nyears)
for(i in 1:nyears){g8.world_PLR_withY_prodpmax[,i] = g8.world_PLR_prodpmax[,i] + GDPoilelast * g8.world_YLR_prodpmax[,i]}
...
g8.worldelast.p.withY <- left_join(g8.worldelast.p.withY, TypeLengthmatch, by = "TypeLength") %>%
  mutate(middle = round(middle, 3)) %>%
  filter(Year == "2017")
...
g8.world.elastp.plot.withY <- ggplot(g8.worldelast.p.withY, aes(x=BaselineType, y=middle)) + 
                  facet_wrap(~TypeLength2) +
                  geom_point(data=g8.worldelast.p.withY, aes(x=BaselineType, y=middle), color = "black") +
...
world.elasts.NAspeed <- allelast.NAspeed %>%
  ...
  mutate(TYPE = as.factor(substr(VAR_SYMBOL, 1,1)),
...
world.elasts.with.ORNLmetareg <- rbind(world.elasts.all, ornl.elasts) %>%
  mutate(elast.in.prod = as.factor(elast.in.prod),
