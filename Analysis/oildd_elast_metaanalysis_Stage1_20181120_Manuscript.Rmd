...
#workdir = "C:/Projects/SPR/oil_elasticities/Journal article" # for Rocio's machine
#workdir = "D:/SPR/2015SPR_LTSR/Data/DemandSupplyElasticities/DElasAnalysis/JournalArticle"  # for Paul's work desktop
...
library(readxl)
library(scales)
library(car) #needed for computing variance inflation factors which help evaluating the extent to which multicollinearity is an issue
library(grid)
library(gridExtra)
...
library(lmtest)
library(sandwich)
library(multiwayvcov) #used to compute cluster-robust standard errors
library(lme4) #used for linear mixed effects estimation
library(ICC) #used for computing intraclass correlation coefficients
...
#allelast <- subset(allelast, STUDY_YR > 1999 & abs(BETA)< 3)
...
# reference.list <- allelast %>%
#   group_by(REFERENCE) %>%
#   summarize(beta.count = sum(BETA))
# write.csv(reference.list, file="reflist.csv")
```
...
paper.periods <- 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "grey"), panel.border = element_rect(colour = "grey", fill=NA, size=1))  
paper.periods 
ggsave("paper.periods.png", width=6.5, height=4.5, units="in", dpi=600)
```
...
g8.forest.plot
ggsave("g8.forest.plot.png", width=6.5, height=4.5, units="in", dpi=600)
...
#Results from WLS estimation    
    ...
    ## Heteroskedasticity
    plot(g8.curr_model2$fitted.values, g8.curr_model2$residuals, title = "Residuals vs. Fitted Values")
...
g8.LRT[[i]] <- tidy(lrtest(g8.curr_model2.lme, g8.curr_model2))
...
g8.baselinesEstimated[[i]] = c(g8.subsetString2, 
                               ...
                               ##LME-related results
                               round(mean(augment(g8.curr_model2.lme)[,".fitted"]),4),
...
g8.coefs.all.clust.se <- add_rownames(as.data.frame(g8.coefs.all.clust.se), "variable")
...
g8.coefs.all.hc.se <- add_rownames(as.data.frame(g8.coefs.all.hc.se), "variable")
...
write.csv(g8.coefs.all.clust.se.ID, "g8.coefs.WLS.csv")
write.csv(g8.coefs.all.LME.ID, "g8.coefs.LME.csv")
write.csv(g8.coefs.all.hc.se.ID, "g8.coefs.OLS.csv")
...
write.csv(g8.all_baselinesEstimated, file = "g8.baselines.csv")
...
funnel.plots <- ggplot(g8.obsSel2.df, aes(x=BETA, y=PRECISION, label=sprintf("%0.2f", round(BETA, digits = 1)))) + 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "grey"), panel.border = element_rect(colour = "grey", fill=NA, size=1))  
funnel.plots 
ggsave("funnel.plots.png", width=6.5, height=4.5, units="in", dpi=600)
...
g8.worldelastw <- read_excel("auxiliary_inputs_20170829.xlsx", sheet = "cons.weights_IEO2013", col_names = TRUE)
g8.worldelastw <- as.data.frame(g8.worldelastw)
...
write.csv(g8.out_summary5, "g8.elast_line_coeffs.csv")
...
write.csv(g8.elast.sim_out[,c(1:4,7:10)], "g8.sim_DDElasts_Out.csv")
```
...
ggsave("g8.world.elastp.plot.withY.png", width=6.5, height=4.5, units="in", dpi=600)
...
ggsave("world.elast.compare.plot.png", width=6.5, height=4.5, units="in", dpi=600)
```
