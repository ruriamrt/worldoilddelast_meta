...
library(styler)
```
The database contains elasticity estimates (and their associated standard errors) from 99 papers. The majority of the papers come from one of the three main energy economics journals. However, the sample also contains working papers and publications in other economics peer-reviewed journals.  
...
This script processes the data and produces exploratory plots.

```{r readData, eval=T, echo=F, include=T, warning=F, message=F}
# Read elasticities database
oilddelastdb <- read_excel("../Data/oilddelasticity_db.xlsx", sheet = "alldata", col_names = TRUE, skip = 1)
...
```{r regionNames, eval=T, echo=F, include=T, warning=F, message=F}
# Create a new region column in the oildemand elasticity database that matches non-standard country groupings/region names to the alternative nomenclatures to be used throughout the analysis
...
# Establish country membership for country groupings used in the studies included in the oil demand elasticity database to then assign values of economic variables (fuel prices, GDP) to these groupings based on the average values for all their members
Country.Groups <- list(
...
The elasticity estimates and supporting information collected from the publications is supplemented by auxiliary variables (trade balance, petroleum product price levels, GDP levels) that can help explain the variability in elasticity estimates for different countries or periods. The auxiliary variables are collected from other sources.  
- Country-level petroleum production and consumption data are collected from EIA and combined to determine if each country is a net exporter or a net importer during the period of analysis used for estimating each elasticity.  
- Country-level retail gasoline and diesel prices are from GTZ surveys conducted during the 1990s. The average year in the data periods of analysis across all publications in the database is in the 1990s. For this reason, a year in that decade provides a representative cross section of prices.     
- Country-level GDP statistics are from Penn World Table, version 9.0f  

```{r readprodconsData, eval=T, echo=F, include=T, warning=F, message=F}
# Read country-level petroleum production and consumption data from EIA International Statistics to determine the trade balance status (net exporter or net importer) for each country.
...
df.netexp.groups <- lapply(Country.Groups, function(x) {
  data.frame(Country = character(34), YEAR = numeric(34), production = numeric(34), consumption = numeric(34), netexp = numeric(34))
})
...
```{r netexports, eval=T, echo=F, include=T, warning=F, message=F}
# Create a new column in the oil elasticities database that displays the appropriate information on net exports and consumption for each elasticity estimate.

# For each elasticity, assign the average net exports for the relevant country/reigon over the analysis period
...
```{r petprodPrices, eval=T, echo=F, include=T, warning=F, message=F}
# Read data on world crude oil price and cross-sections of petroleum product prices (retail price levels inclusive of taxes or subsidies)
...
Based on the number of countries for which data are available in the GTZ survey, the 1998 gasoline price cross section (instead of 1995 or 1991 alternatives) will be the one used in the metaregressions.

```{r GDPdata, eval=T, echo=F, include=T, warning=F, message=F}
# Read country-level GDP per capita data
...
For long-run elasticity estimates obtained from partial adjustment models, error correction models, and other ARDL models, it is possible to calculate the length of the adjustment period captured by the elasticity. We perform the calculations necessary to compute length of run (in years) for long-run elasticities obtained from publications using those specifications for their demand equations.  

Partial adjustment model:  
$$y_{t} = a + d*y_{t-1} + f*x_{t} + e_{t}$$
Error correction model:  
$$y_{t} - y_{t-1} = a + (b-1)*(y_{t-1} - c*x_{t-1}) + d*(x_{t} - x_{t-1}) + u_{t}$$
...
```{r lengthrunCalc, eval=T, echo=F, include=T, warning=F, message=F}
...
oilddelastdb2 <- oilddelastdb %>%
  ...
  mutate(
    LAGGEDD = ifelse(LENGTH_RUN == "multiperiod", BETA[VAR_SYMBOL %in% lagged.varsymbols], NA),
...
Not all publications include the standard errors for the estimated coefficients from their demand equations. However, out of the ones that do not report standard errors, many contain another measure of the precision of the estimate (e.g., confidence interval bounds, t-statistic) that can used to derive an approximate standard error. However, some other meta-analysis that have used this same approach to calculate standard errors from other precision measures found that calculated standard errors tend to be larger than those directly reported in the papers. As a visual test for whether bias appears to be present in this case, we plot the ratio of reported standard error to calculated standard error for a set of papers that contain sufficient information to compare both. 

```{r StErrorCalcTests, eval=T, echo=F, include=T, warning=F, message=F}
# Tests to ascertain whether calculated standard errors appear to be biased relative to those directly reported in the papers

## Select elasticities for which papers contain estimates of standard errors and enough information to calculate a standard error.
...
oilddelastdb.sterr.recalc.4 <- oilddelastdb.sterr.recalc %>%
  ...
  mutate(
    BETA_PSR = ifelse(MODEL_TYPE == "log-linear, partial adjustment model", BETA[VAR_SYMBOL == "Pt"], NA),
...
oilddelastdb.sterr.recalc.diff.plot <- ggplot(oilddelastdb.sterr.recalc.summ, aes(x = REF.EQ, y = VALUE)) +
  ...
  theme(panel.grid.major.x = element_blank(), panel.grid.major.y = element_line(linewidth = 0.25, color = "grey"), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "grey"), panel.border = element_rect(colour = "grey", fill = NA, size = 1))
oilddelastdb.sterr.recalc.diff.plot
```


``` {r elastOnlyDB, eval=T, echo=F, include=T, warning=F, message=F}
# Extract subset of rows that contain elasticity estimates (this will be the subset used for the metaregression analysis)
...
Next, we produce a set of plots to explore the elasticity dataset.  
- P_SR: short-run price elasticities  
- P_LR: long-run price elasticities  
- Y_SR: short-run income elasticities  
- Y_LR: long-run income elasticities

``` {r defineElasLengthVars, eval=T, echo=F, include=T, warning=F, message=F}
...
allelast <- allelast %>%
  mutate(
    TYPE_LENGTH = paste0(TYPE, "_", LENGTH),
...
hist.typelength <- ggplot(allelast, aes(x = BETA)) +
  ...
  labs(y = "count", x = "Elasticity", title = "Elasticity Histograms \nby Type and Length of Run") +
  ...
  theme(plot.title = element_text(size = 14, face = "bold", hjust = 0.5)) +
  theme(panel.grid.major.y = element_line(linewidth = .1, color = "grey"), panel.grid.major.x = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), panel.border = element_blank(), axis.line = element_line(colour = "black"))
hist.typelength
...
elast.lengthrun.plot <- ggplot(allelast[allelast$TYPE_LENGTH2 == "P_LR", ], aes(x = SPEEDYRS, y = BETA)) +
  ...
  labs(y = "Elasticity", x = "Length of run (years)", title = "Long-Run Price Elasticities versus \nLength of Adjustment Period") +
  theme(axis.text.x = element_text(size = 12), axis.text.y = element_text(size = 12), axis.title = element_text(size = 12)) +
...
```{r elastbyNetExp, eval=T, echo=F, include=T, warning=F, message=F}
# boxplot showing differences in distribution of elasticities for net exporting versus net importing countries
allelast <- allelast %>%
  mutate(
    NETEXP = ifelse(avg_NETEXP <= 0, "Net_Importers", "Net_Exporters"),
...
elast.netexp.bplot <- ggplot(allelast, aes(x = NETEXP, y = BETA)) +
  ...
  labs(x = "", y = "Elasticity", title = "Elasticity distributions \nfor importing vs. exporting countries") +
  theme(strip.text.x = element_text(size = 12)) +
...
```{r product_sector, eval=T, echo=F, include=T, warning=F, message=F}
# Plots to examine difference in elasticities by petroleum product or end-use sector
...
Ideally, we would like to classify all product types/sectors into the categories provided in EIA's International Energy Outlook (in order to project evolution of elasticities over time): Residential, Commercial, Industrial, Transportation, Electricity, Total.
...
```{r region, eval=T, echo=F, include=T, warning=F, message=F}
# Plots to examine differences in elasticities across regions
...
```{r avgyr, eval=T, echo=F, include=T, warning=F, message=F}
...
elast.avgyr.plot <- ggplot(allelast, aes(x = AVGYR, y = BETA, color = LENGTH)) +
  ...
  labs(x = "", y = "Elasticity", title = "Elasticity vs. \nAverage Year of Analysis Period") +
  theme(strip.text.x = element_text(size = 12)) +
  theme(plot.title = element_text(size = 18, face = "bold", hjust = 0.5)) +
  theme(axis.text.x = element_text(size = 12, angle = 90, hjust = 1), axis.text.y = element_text(size = 12), axis.title = element_text(size = 14, face = "bold")) +
  theme(legend.text = element_text(size = 12), legend.title = element_blank(), legend.key = element_blank(), legend.position = "right") +
...
```{r methods, eval=T, echo=F, include=T, warning=F, message=F}
# Plots to examine differences in elasticities across estimation methods
...
Pmax denotes elasticity with respect to the price maximum (instead of the average elasticity across all prices in the period) observed during the period of analysis used to estimate the demand equation. It is typically expected that the elasticity with respect to a new price maximum is higher than the elasticity with respect to all price levels.  

```{r pmaxPlot, eval=T, echo=F, include=T, warning=F, message=F}
# Plot showing difference in elasticities with respect to average price versus maximum prices
...
allelast.Pmax.summary.w <- allelast.Pmax.summary.Pmax %>%
  right_join(allelast.Pmax.summary.Psubmax, by = c("REFERENCE", "LENGTH", "PRODUCT_SECTOR", "REGION", "label")) %>%
  filter(!is.na(average.x) & !is.na(average.y)) %>%
  mutate(
    abs.diff = average.x - average.y,
...
```{r saveElastDB, eval=T, echo=F, include=T}
summary(allelast)
